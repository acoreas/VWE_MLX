...
// #endif
	...
	for (CurZone = 0; CurZone < ZoneCount; CurZone++)
	{
		...
		// 		NextR = ((1 - DT * 0.5 * OneOverTauSigma) * ELD(Rxx, index) - dFirstPartForR + MiuComp * (Dy)) / (1 + DT * 0.5 * OneOverTauSigma);
		...
		// 	if (bAttenuating == 1)
		// 	{
		// 		NextR = ((1 - DT * 0.5 * OneOverTauSigma) * ELD(Ryy, index) - dFirstPartForR + MiuComp * (Dx)) / (1 + DT * 0.5 * OneOverTauSigma);
		...
		// 			NextR = ((1 - DT * 0.5 * OneOverTauSigma) * ELD(Rxy, index) - DT * MiuComp * Dx) / (1 + DT * 0.5 * OneOverTauSigma);
